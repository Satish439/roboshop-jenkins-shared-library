...
def publishArtifacts() {
    ...
    stage('Push Artifacts to Nexus') {
        withCredentials([usernamePassword(credentialsId: 'NEXUS', passwordVariable: 'password', usernameVariable: 'username')]) {
            sh """
        curl -u ${USERNAME}:${PASSWORD} --upload-file ${ENV}-${COMPONENT}-${TAG_NAME}.zip http://3.91.226.208:8081/repository/${COMPONENT}/${ENV}-${COMPONENT}-${TAG_NAME}.zip
      """
        }
    }
        ...
        parallel([
                qualityChecks: {
                    withCredentials([usernamePassword(credentialsId: 'SONAR', passwordVariable: 'pass', usernameVariable: 'user')]) {
                        sh "sonar-scanner -Dsonar.projectKey=${COMPONENT} -Dsonar.host.url=http://52.201.103.29:9000 -Dsonar.login=${user} -Dsonar.password=${pass}"
